...
export default {
  Mutation: {
    getOrCreateChannel: requiresAuth.createResolver(async (parent, { teamId, members }, { models, user }) => {
      ...
      // check if channel already exitsts with these members
      const [data] = await models.sequelize.query(
        `
        select c.id, c.name, json_arrayagg(pc.user_id) as pcids
        from channels as c, pcmembers as pc
        where pc.channel_id = c.id and c.dm = true and c.public = false and c.team_id = ${teamId}
        group by c.id, c.name
        having json_contains(pcids, '[${allMembers}]') and count(pc.user_id) = ${allMembers.length}
      `,
        {
          raw: true
        }
